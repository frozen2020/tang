...
void Initialize()
{
	...
	*XINT2CR = 0x8000;
	...
	if( (LASSET > 450) || (LASSET < 150) ) {
		LASSET = 350;
		wrf = 1;
	}
	/* TEMPTBL1[0] = 15 Degrees C, TEMPTBL1[0] = 32 Degrees C */
	if( (ENVSET > 550) || (ENVSET < 150) ) {//++xiao
		ENVSET = 400;//++xiao
		wrf = 1;
	}
...
//	FPS_ON=0;
	...
	*PEDATDIR = *PEDATDIR & 0xFFBF; 	//init of FPS is off//--xiao
	//*PEDATDIR = *PEDATDIR | 0x0040; //++xiao
...
void num2ascii(unsigned long number, short chr)
{
	...
	if(chr == 1) {
	    ...
	    number = (120 - number) / 1;           //500ns//y=(340-x)/3;Limit 40,310
	    if(number / 100)
	    HOLD[0] = '1';
	    else
	    HOLD[0] = ' ';
...
void tempctl(void)
{
	...
	if( !(*ADCTRL2 & 0x1000) )    // MAKE SURE ADC READY
		...
		T3 =		((((*RESULT3>>6)+(*RESULT11>>6))>>1) & 0x3FF);
		T4 =		((((*RESULT4>>6)+(*RESULT12>>6))>>1) & 0x3FF);
		T5 =		((((*RESULT5>>6)+(*RESULT13>>6))>>1) & 0x3FF);
...
*/			
	...
	D1_OUT_OLD = D1_OUT;	
	*T1CMPR = D1_OUT; //8/29
	ADTIMER = 2000;				/* reset timer 100 milliseconds*/
} 
...
void WriteData()
{short ENVSET2,i,QSWEN2,DIMAX2,LASSET2,XTL2SET2;
 float D1_SET2;
...
while(i<3)
{
  if( PULSERATE2!=PULSERATE||PWMTime2!=PWMTime||ENVSET2!=ENVSET||DIMAX2!=DIMAX||XTL2SET2!=XTL2SET||LASSET2 != LASSET||QSWEN2 != QSWEN   )

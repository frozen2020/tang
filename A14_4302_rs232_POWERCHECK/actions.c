...
/*   ENTER FUNCTION DATA   */
	...
	else if(FUNCTION == 10) POWERSET= ADJUST;
...
/*   FUNCTION SELECT INCREMENT   */
	...
	if(!(*pFuncFlg))
		...
		else if( FUNCTION == 10 ) ADJUST = POWERSET;
  	...
  	while( (!(*PFDATDIR & 0x0001)) && (FUNCTION > 1)&&(*PFDATDIR & 0x0002)) // ++)
   	...
   	//************************************************************************
		...
		{if(ADJUST < LAS_MINTEMP) ADJUST = LAS_MINTEMP;        
	 	...
	 	if(FUNCTION == 10)
		{
			if(ADJUST < 0) ADJUST = 0;        //400
   		else if(ADJUST > 1000) ADJUST = 1000;
		}
   		//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   		if((FUNCTION==2)&&(MODE_SEL==1))
   		{

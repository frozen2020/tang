...
void ReadDatax()
{
	...
	*PFDATDIR = (*PFDATDIR & ~0x0040);		/* ENABLE TIMEKEEPER */
	...
	WriteDallas2(READ,1);//op_code
	WriteDallas2(0,1); 	//write address A11~A0
	WriteDallas2(0,1);	

	XTL1SET = ReadDallas2(0 );
	DIMAX = ReadDallas2( 0);        
	i = ReadDallas2( 0);
	...
	PR |= i;
	PULSERATE = PR;
	i1 = ReadDallas2( 0);  //PWM
	PR1 = ((unsigned long)i1<<16);
	i1 = ReadDallas2(0 );
	PR1 |= i1; 
	PWMTime = PR1; 
	D2_SET = ReadDallas2(0 );
	ENVSET = ReadDallas2(0 );
	D1_SET = ReadDallas2(0 );
	Time = ReadDallas2(0 );
	//XTL2SET = ReadDallas2(0 );
	FPS_ON = ReadDallas2(0 );      //   ++
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	INITCUR= ReadDallas2(0 );
	INITCUR2= ReadDallas2(0 );
	securityenable= ReadDallas2(0 );
	LASSET = ReadDallas2(0 );
	//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	*PFDATDIR = (*PFDATDIR | 0x0040);		/*DISABLE TIMEKEEPER */
	*MCRB = 0xFF0C;
	*EVAIMRA = 0x0080;         /* enable desired EVA group A interrupts  tang*/
...
/*6. ReadDallas2 */
unsigned short ReadDallas2(short Command )
{
	unsigned short Tdata[2];
	unsigned short Data;
	int i,j;	

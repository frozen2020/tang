...
void Initialize()
{
	...
	*PFDATDIR = (*PFDATDIR | 0x0040);		/* ENABLE TIMEKEEPER */
	WriteDallas2(0x8F, 1);		// Write command
	WriteDallas2(0, 1);		// Write command
	WriteDallas2(0x91,1); /*往涓流充电寄存器写0xa5*/
	WriteDallas2(0xa5,1); 
...
void WriteData()
{
	...
	*MCRB = 0xFF1C;
	*PFDATDIR = (*PFDATDIR | 0x0040);		/* ENABLE TIMEKEEPER */
	WriteDallas2(0xa0, 1);		// Write command
...
void ReadData()
{
	...
	*PFDATDIR = (*PFDATDIR | 0x0040);		/* ENABLE TIMEKEEPER */
	...
	WriteDallas2(0x20, 1);		// Write command
	XTL1SET = ReadDallas2( );
	DIMAX = ReadDallas2( );        
	i = ReadDallas2( );

...
void dspfun()
{
	...
	//FPS//
		...
		message(msg33);
	 	PLACE = 17;
				if(POS_OLD>7)
			num2ascii(POS_OLD-7, 8); 
			else 
			num2ascii(POS_OLD+9, 8); 
	 //	PLACE = 37;
	//	num2ascii(POS_NEW,8);
	 	}
} 
...
void tempctl(void)
{
	...
	*CMPR2 = (2000 - ((XTL2SET - 260) * 5.88));
	...
	LASERPOWER = (float)T2*33*3*2 *10/0x3FF;  //tang
    /*
    if(*PBDATDIR & 0X0040)
    	{
...
//rev6		num2ascii(DIODECUR2,0);
	...
	if(SIGN_FUN8SET==4)
		...
		else if (FUNCTION == 4) 
		{
			...
			num2ascii(POS_OLD+9, 8); 	 	
			if((POS_NEW>7 )&&(POS_NEW<14))
			PLACE= 66+ (POS_NEW-8)*3;
			else if((POS_NEW>13 )&&(POS_NEW<17))
			PLACE= 22+ (POS_NEW-14)*3;
			else if((POS_NEW>0 )&&(POS_NEW<4))
			PLACE= 31 +(POS_NEW-1) *3;
			else if((POS_NEW>3 )&&(POS_NEW<8))
			PLACE= 89 +(POS_NEW-4) *3;
			wait(20);
				port00 = PLACE+128;
	 			wait(20);
	 			port00=0x000F;  //turn off cursor and blink 
